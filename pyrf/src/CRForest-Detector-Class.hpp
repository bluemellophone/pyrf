...
public:
    ...
    CRForest *forest(vector<string> &tree_path_vector, bool verbose)
    {
        // Init forest with number of trees
        CRForest *crForest = new CRForest( tree_path_vector.size() );
        // Load forest
        crForest->loadForest(tree_path_vector, verbose);
        return crForest;
    }
    ...
    void train(string train_pos_chip_path_string,
               ...
               float trees_prob_optimize_mode, bool verbose)
    {
               ...
               bool verbose)
    {
        ...
        IplImage *combined = cvCreateImage(cvSize(img->width, img->height), IPL_DEPTH_16U, 1);
        IplImage *upscaled = cvCreateImage(cvSize(img->width, img->height), IPL_DEPTH_16U, 1);
        ...
        crDetect.detectPyramid(img, vImgDetect, manifests, mode);
        ...
        for (k = 0; k < vImgDetect.size(); k++)
        {
            IplImage *scaled = cvCreateImage(cvSize(vImgDetect[k]->width, vImgDetect[k]->height), IPL_DEPTH_16U, 1);
            cvConvertScale( vImgDetect[k], scaled, 255);

            // Add scale to combined
            cvResize(scaled, upscaled);
        ...
        cvConvertScale( combined, output, 255.0 / maxval );
        ...
        if(mode == 0)
        {
            ...
            // Threshold the image
            cvThreshold(output, output, int(255 * (1.0 - sensitivity)), 0, CV_THRESH_TOZERO);
            // DEBUG
            if (output_gpath.length() > 0)
            {
                // Save output mode image
            ...
            {    
                ...
                if(rect.width * rect.height >= nms_min_area_contour)
                {
                    ...
                    {
                        minx = (int) (centerx - rect.width)  * scale_vector[k];
                        maxx = (int) (centerx + rect.width)  * scale_vector[k];
                        miny = (int) (centery - rect.height) * scale_vector[k];
                        maxy = (int) (centery + rect.height) * scale_vector[k];
                        for(y = miny; y < maxy; ++y)
                        {
                            for(x = minx; x < maxx; ++x)
                            {
