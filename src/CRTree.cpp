...
bool CRTree::saveTree(const char* filename) const {
	...
	// ofstream monitor("output.txt");
	if(out.is_open()) {

		out << max_depth << " " << num_leaf << " " << num_cp << endl;
		// monitor << "TreeDepth leafs centers" << endl;
		...
		for(unsigned int l=0; l<num_leaf; ++l, ++ptLN) {
			...
			// monitor << "Leaf ratio size vector[";
...
// Start grow tree
void CRTree::growTree(const CRPatch& TrData, int samples, int default_split) {
	...
	// Grow tree
	grow(TrainSet, 0, 0, samples, pos / float(TrainSet[0].size()), default_split );
}

// Called by growTree
void CRTree::grow(const vector<vector<const PatchFeature*> >& TrainSet, int node, unsigned int depth, int samples, float pnratio, int default_split) {
	...
	if(depth<max_depth && TrainSet[1].size()>0) {	
		...
		// Set measure mode for split: 0 - classification (class-label), 1 - regression (offset)
		unsigned int measure_mode = 1;

		if( default_split < 0 || default_split > 100 )
		{
		...
		{	
			if( cvRandInt( cvRNG ) % 100 < default_split )
				measure_mode = 0;
			else
				measure_mode = 1;
		}
		...
		if( optimizeTest(SetA, SetB, TrainSet, test, samples, measure_mode) ) {
			...
			if(SetA[0].size()+SetA[1].size()>min_samples) {
				grow(SetA, 2*node+1, depth+1, samples, pnratio, default_split);
			} else {
			...
			if(SetB[0].size()+SetB[1].size()>min_samples) {
				grow(SetB, 2*node+2, depth+1, samples, pnratio, default_split);
			} else {
