...
void CRPatch::extractPatches(IplImage *img, unsigned int n, int label, CvRect* box, std::vector<CvPoint>* vCenter, bool legacy) {
	// if(box->x + box->width > img->width || box->y + box->height > img->height)
	// {	
	// 	cout << "ABORTING: " << box->x + box->width << " " << img->width << " | " << box->y + box->height << " " << img->height << endl;
	// 	return;
	// }

	// extract features
	vector<IplImage*> vImg;
	extractFeatureChannels(img, vImg, legacy);

	CvMat tmp;
	int offx = width/2; 
	int offy = height/2;
	
	int area = (box->width * box->height * n) / (width * height);
	// cout << "    Area: " << area << endl;

	// generate x,y locations
	CvMat* locations = cvCreateMat( area, 1, CV_32SC2 );
	if(box==0)
	...
	vLPatches[label].reserve(offset+area);
...
void CRPatch::extractFeatureChannels(IplImage *img, std::vector<IplImage*>& vImg, bool legacy) {
