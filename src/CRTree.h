...
public:
	...
	// Training
	void growTree(const CRPatch& TrData, int samples, int default_split);
...
private: 
	...
	void grow(const std::vector<std::vector<const PatchFeature*> >& TrainSet, int node, unsigned int depth, int samples, float pnratio, int default_split);
