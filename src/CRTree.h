...
public:
    ...
    CRTree(int min_s, int max_d, int pw, int ph, CvRNG *pRNG) : min_samples(min_s), max_depth(max_d), num_leaf(0), patch_width(pw), patch_height(ph), cvRNG(pRNG) {
    ...
    // Training
    void growTree(const CRPatch &TrData, int samples, float split, bool verbose);
...
private:
    ...
    void grow(const std::vector<std::vector<const PatchFeature *> > &TrainSet, int node, unsigned int depth, int samples, float pnratio, float split, bool verbose);
    ...
    // patch width used for training
    unsigned int patch_width;

    // patch height used for training
